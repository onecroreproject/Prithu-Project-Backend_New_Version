...
/* -------------------------------------------------------
|--------------------------------------------------------------------------
| 2️⃣ ADMIN FEED UPLOAD (MULTIPLE FILES: image/video) - WITH BUFFER
|--------------------------------------------------------------------------
---------------------------------------------------------*/
...
// Check duplicates & duration - READ FILE BUFFER
const adminProcessFeedFile = async (req, res, next) => {
  ...
  try {
    for (const file of files) {
      // Read file buffer for hash and processing
      const buffer = fs.readFileSync(file._savedPath);
      file._buffer = buffer; // Store buffer for Google Drive upload
...
// Attach feed files list with buffer
const attachAdminFeedFiles = (req, res, next) => {
  ...
  req.localFiles = files.map((file) => ({
    ...
    folder: file._folder,
    buffer: file._buffer, // Include buffer for Google Drive
